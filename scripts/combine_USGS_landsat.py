...
from codebase.dataprocessing import combine_landsat_geotiffs
import os, re
...
# Pull dates from all Landsat files   # Use regex to pull the date from all files starting with 'HLS' (Landsat instrument)
all_dates = [re.search(search_pattern, i_file).group(0) for i_file in all_files if i_file.startswith('HLS')]
