...
# def test_DEM_shape():
#     expected_bbox = (9001,36001)
#     output_dem , output_lat ,output_lon = load_DEM_subset(bbox)
#     output_shape = output_dem.shape
#     np.testing.assert_equal(output_shape, expected_shape)
#     np.testing_assert_equal(len(output_lat),expected_shape[0])
#     np.testing_assert_equal(len(output_lon),expected_shape[1])
...
#     np.testing_assert_allclose(output_stats,expected_stats,rtol = 1e-2)